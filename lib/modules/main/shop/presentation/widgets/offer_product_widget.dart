  ...
  Widget build(BuildContext context) {
    ...
    return BlocBuilder<ShopBloc, ShopState>(
      builder: (context, state) {
        bool isLoading = state.offerProdStatus == Status.loading ||
            state.offerProdStatus == Status.sleep;
        return Column(
          children: [
            isLoading
                ? Padding(
                ...
                : Visibility(
                    visible: state.offerProds.isNotEmpty,
                    child: Padding(
              ...
              child: IntrinsicHeight(
                child: Row(
                  children: List.generate(
                    isLoading ? 5 : state.offerProds.length,
                    (index) => Container(
                      width: 1.sw / 2.2,
                      margin: arabic
                      ...
                      child: isLoading
                          ...
                          : ProductWidget(product: state.offerProds[index]),
                    ),
                  ),
                ),
              ),
